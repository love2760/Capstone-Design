...
Java_ce_inu_ikta_imgprocessor_imgFiltering(JNIEnv *env, jobject instance, jlong matAddrInput) {
    ...
    findContours(src, contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_TC89_KCOS, Point());
	...
	vector<Mat> images(contours_poly.size());
	images = extractContours(src, contours_poly);
}

vector<Mat> extractContours(Mat& image, vector< vector<Point> > contours_poly)
{
	vector<Mat> images(contours_poly.size());
